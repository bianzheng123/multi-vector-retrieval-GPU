...
namespace VectorSetSearch
{
    ...
    {
    public:
        std::vector<float> score_l_; // query_n_vec * n_item * max_item_n_vec
        std::vector<float> max_score_l_; // query_n_vec * n_item
                    ...
                    const uint32_t& max_item_n_vec)
        {
            ...
            score_l_.resize(n_item_ * query_n_vec_ * max_item_n_vec_);
            max_score_l_.resize(n_item_ * query_n_vec_);
        }
                     ...
                     std::pair<float, uint32_t>* max_sim_score_l)
        {
            ...
            MatrixMultiply(query, item_l, query_n_vec_,
                           n_vec, vec_dim, score_l_.data());

            // then perform the max operator
            for (uint32_t qID = 0; qID < query_n_vec_; qID++)
            {
                const float* query_score_l = score_l_.data() + qID * n_vec;
                float* query_max_score_l = max_score_l_.data() + qID * n_item_;
            ...
            for (uint32_t qID = 0; qID < query_n_vec_; qID++)
            {
                const float* query_max_score_l = max_score_l_.data() + qID * n_item_;
                for (uint32_t itemID = 0; itemID < n_item_; itemID++)
                {
