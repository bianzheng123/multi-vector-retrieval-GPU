...
int main(int argc, char** argv) {
  ...
  VectorSetSearch::Method::IGPGPUBasic index(item_n_vec_l_fname);
  ...
  const VectorSetSearch::Method::IGPGPUBasicResult result = index.search(
  ...
  sprintf(retrieval_suffix_str, "nprobe_%d-probe_topk_%d", nprobe, probe_topk);
  ...
  if (is_profile) {
    sprintf(performance_filename,
            "%s/%s-%s-performance-top%d-%s-%s-profile.tsv",
            answer_path.c_str(), dataset.c_str(), method_name.c_str(),
            topk, build_index_suffix.c_str(), retrieval_suffix_str);
  } else {
  ...
  performance_out <<
      "search_time\treset_time\ttransmit_query_time\tcand_gen_time\tfilter_time\tfilter_transmit_time\t"
      <<
      "filter_compute_time\trerank_time\trerank_transmit_time\trerank_compute_time"
      << std::endl;
  for (uint32_t qID = 0; qID < n_query; qID++) {
    performance_out << result.compute_time_l[qID] << "\t"
        << result.reset_time_l[qID] << "\t"
        << result.transmit_query_time_l[qID] << "\t"
        << result.cand_gen_time_l[qID] << "\t"
        << result.filter_time_l[qID] << "\t"
        << result.filter_transmit_time_l[qID] << "\t"
        << result.filter_compute_time_l[qID] << "\t"
        << result.rerank_time_l[qID] << "\t"
        << result.rerank_transmit_time_l[qID] << "\t"
        << result.rerank_compute_time_l[qID] << std::endl;
  }
