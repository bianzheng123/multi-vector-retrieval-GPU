...
def approximate_solution_retrieval(username: str, dataset: str,
    ...
    search_time_m = {
        ...
        "reset_time_average(ms)": '{:.3f}'.format(1.0 * np.average(performance_df['reset_time']) * 1e3),
        "transmit_query_time_average(ms)": '{:.3f}'.format(1.0 * np.average(performance_df['transmit_query_time']) * 1e3),
        "cand_gen_time_average(ms)": '{:.3f}'.format(1.0 * np.average(performance_df['cand_gen_time']) * 1e3),
        "filter_time_average(ms)": '{:.3f}'.format(1.0 * np.average(performance_df['filter_time']) * 1e3),
        "filter_transmit_time_average(ms)": '{:.3f}'.format(1.0 * np.average(performance_df['filter_transmit_time']) * 1e3),
        "filter_compute_time_average(ms)": '{:.3f}'.format(1.0 * np.average(performance_df['filter_compute_time']) * 1e3),
        "rerank_time_average(ms)": '{:.3f}'.format(1.0 * np.average(performance_df['rerank_time']) * 1e3),
        "rerank_transmit_time_average(ms)": '{:.3f}'.format(1.0 * np.average(performance_df['rerank_transmit_time']) * 1e3),
        "rerank_compute_time_average(ms)": '{:.3f}'.format(1.0 * np.average(performance_df['rerank_compute_time']) * 1e3),
    }
...
if __name__ == '__main__':
    ...
    config_l = {
        'dbg': {
            ...
            # 'dataset_l': ['lotte', 'msmacro', 'hotpotqa', 'wiki-nq'],
            'dataset_l': ['lotte', 'hotpotqa', 'msmacro'],
            'topk_l': [10, 100],
            'is_debug': False,
        ...
        'local': {
            ...
            'retrieval_parameter_l': [
                ...
                {'nprobe': 64, 'probe_topk': 20},
                # {'nprobe': 128, 'probe_topk': 20},
                # {'nprobe': 256, 'probe_topk': 20},

                # {'nprobe': 64, 'probe_topk': 40},
                # {'nprobe': 64, 'probe_topk': 80},
                # {'nprobe': 64, 'probe_topk': 120},
                # {'nprobe': 64, 'probe_topk': 160},
            ],
            'grid_search': True,
            'grid_search_para': {
                10: {
