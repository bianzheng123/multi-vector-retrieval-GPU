...
def generate_data_by_sample(username: str, origin_dataset: str, new_dataset: str,
    ...
    raw_data_base_path = f'/home/{username}/Dataset/multi-vector-retrieval-GPU/RawData'
...
def build_basic_index(username: str, origin_dataset: str, new_dataset: str, n_sample_item: int, n_sample_query: int,
                      topk_l: list,
                      n_centroid_f: str, n_bit: int, sample_pid_f: str):
    embedding_path = f'/home/{username}/Dataset/multi-vector-retrieval-GPU/Result/Embedding'
    os.system(f'rm -r {embedding_path}/{new_dataset}')

    index_path = f'/home/{username}/Dataset/multi-vector-retrieval-GPU/Result/Index'
    os.system(f'rm -r {index_path}/{new_dataset}')

    rawdata_path = f'/home/{username}/Dataset/multi-vector-retrieval-GPU/Result/RawData'
    ...
    n_centroid, n_pid_kmeans = colbert_run.build_index_official(username=username, dataset=new_dataset,
                                                                n_centroid_f=n_centroid_f,
                                                                n_bit=n_bit, sample_pid_f=sample_pid_f,
                                                                save_embedding=True)
    ...
    return n_centroid, n_pid_kmeans
...
if __name__ == '__main__':
    ...
    config_l = {
        'dbg': {
            ...
            'sample_dataset_info_l': [
                {'origin_dataset': 'lotte', 'new_dataset': 'lotte-100K',
                 'n_sample_item': 100_000, 'n_sample_query': -1,
                ...
                {'origin_dataset': 'msmacro', 'new_dataset': 'msmacro-100K',
                 'n_sample_item': 100_000, 'n_sample_query': -1,
                 'n_centroid_f': 'lambda n_vec: int(2 ** np.floor(np.log2(16 * np.sqrt(n_vec))))', 'n_bit': 2,
                 'sample_pid_f': 'lambda typical_doclen, num_passages: 16 * np.sqrt(typical_doclen * num_passages)'},
            ]
        ...
        'local': {
            ...
            'sample_dataset_info_l': [
                {'origin_dataset': 'lotte', 'new_dataset': 'lotte-500-gnd',
                 'n_sample_item': 500, 'n_sample_query': 100,
    ...
    for sample_dataset_info in sample_dataset_info_l:
        origin_dataset = sample_dataset_info['origin_dataset']
        new_dataset = sample_dataset_info['new_dataset']
        n_sample_item = sample_dataset_info['n_sample_item']
        n_sample_query = sample_dataset_info['n_sample_query']
        n_centroid_f = sample_dataset_info['n_centroid_f']
        n_bit = sample_dataset_info['n_bit']
        sample_pid_f = sample_dataset_info['sample_pid_f']

        queries_gnd_json_l_filename = os.path.join(raw_data_path,
                                                   f'{origin_dataset}/document/queries.gnd.jsonl')
        queries_short_answer_gnd_json_l_filename = os.path.join(raw_data_path,
                                                                f'{origin_dataset}/document/queries_short_answer.gnd.jsonl')
        ...
        n_centroid, n_pid_kmeans = build_basic_index(username=username, origin_dataset=origin_dataset,
                                                     new_dataset=new_dataset,
                                                     n_sample_item=n_sample_item, n_sample_query=n_sample_query,
                                                     topk_l=topk_l,
                                                     n_centroid_f=n_centroid_f, n_bit=n_bit, sample_pid_f=sample_pid_f)
