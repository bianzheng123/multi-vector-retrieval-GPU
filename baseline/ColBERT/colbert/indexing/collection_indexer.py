...
def encode(config, collection, embedding_filename, save_embedding, shared_lists, shared_queues):
    encoder = CollectionIndexer(config=config, collection=collection)
    build_index_time, encode_passage_time, n_centroid, n_pid_kmeans = encoder.run(embedding_filename, save_embedding, shared_lists)
    return build_index_time, encode_passage_time, n_centroid, n_pid_kmeans
...
class CollectionIndexer():
    ...
    def run(self, embedding_filename, save_embedding, shared_lists):
        with torch.inference_mode():
            start_time = time.time()
            n_pid_kmeans = self.setup()  # Computes and saves plan for whole collection
            ...
            start_time = time.time()
            encode_passage_time = self.index(embedding_filename, save_embedding)  # Encodes and saves all tokens into residuals
            distributed.barrier(self.rank)
        ...
        return build_index_time, encode_passage_time, self.num_partitions, n_pid_kmeans
    ...
    def setup(self):
        ...
        sampled_pids, n_sample_pid = self._sample_pids()
        ...
        self.num_partitions = eval(self.config.n_centroid_f)(self.num_embeddings_est)
    ...
    def _sample_pids(self):
        ...
        # current used: sampled_pids = 16 * np.sqrt(typical_doclen * num_passages)
        sampled_pids = eval(self.config.sample_pid_f)(typical_doclen, num_passages)
        ...
        return set(sampled_pids), len(sampled_pids)
    ...
    def index(self, embedding_filename, save_embedding):
        '''
        ...
        with self.saver.thread():
            ...
            for chunk_idx, offset, passages in tqdm.tqdm(batches, disable=self.rank > 0):
                ...
                # @bianzheng
                start_time = time.time()
                if embedding_filename is not None and save_embedding:
                    import numpy as np
