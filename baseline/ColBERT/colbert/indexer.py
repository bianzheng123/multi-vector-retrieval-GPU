...
class Indexer:
    def __init__(self, checkpoint, config=None):
        ...
        # print(f"self.checkpoint_config.kmeans_niters, config.kmeans_niters {self.checkpoint_config.kmeans_niters}, {config.kmeans_niters}")
        # print(f"self.checkpoint_config.nbits, config.nbits {self.checkpoint_config.nbits}, {config.nbits}")

        self.configure(checkpoint=checkpoint)
        print(f"self.config.nbits, self.config.kmeans_niters, {self.config.nbits}, {self.config.kmeans_niters}, {self.config.n_centroid_f}")
    ...
    def index(self, name, collection, embedding_filename: str = None, save_embedding: bool=True, overwrite=False):
        ...
        if index_does_not_exist or overwrite != 'reuse':
            build_index_time, encode_passage_time, n_partition, n_pid_kmeans = self.__launch(collection, embedding_filename, save_embedding)

        return build_index_time, encode_passage_time, n_partition, n_pid_kmeans

    def __launch(self, collection, embedding_filename: str, save_embedding:bool):
        manager = mp.Manager()
        ...
        build_index_time, encode_passage_time, n_partition, n_pid_kmeans = launcher.launch(self.config, collection,
                                                                             embedding_filename, save_embedding, shared_lists, shared_queues)
        return build_index_time, encode_passage_time, n_partition, n_pid_kmeans
